d_nl <- d_nl %>% 
  ...
  mutate(D2 = recode(D2,
         ...
         stance1 = ifelse(specified1 == "Specified" & S1 == 2, 
                          "correct", "not correct"),
         stance1 = ifelse(specified1 == "Underspecified" & S1 == 3, 
                          "correct", stance1),
         stance2 = ifelse(specified2 == "Specified" & S2 == 1, 
                          "correct", "not correct"),
         stance2 = ifelse(specified2 == "Underspecified" & S2 == 3,
                          "correct", stance2),
         stance3 = ifelse(specified3 == "Specified" & S3 == 1,
                          "correct", "not correct"),
         stance3 = ifelse(specified3 == "Underspecified" & S3 == 3, 
                          "correct", stance3),
         stance4 = ifelse(specified4 == "Specified" & S4 == 2, 
                          "correct", "not correct"),
         stance4 = ifelse(specified4 == "Underspecified" & S4 == 3,
                          "correct", stance4),
  ...
  select(id, D1:D6, PT1:PT6, PT7, attention,
         actor1:actor4,
         issue1:issue4,
         specified1:specified4, stance1:stance4,
         interpret1:interpret4) %>% 
  mutate(round1 = paste(actor1, issue1, specified1, stance1, interpret1, sep = "-"),
         round2 = paste(actor2, issue2, specified2, stance2, interpret2, sep = "-"),
         round3 = paste(actor3, issue3, specified3, stance3, interpret3, sep = "-"),
         round4 = paste(actor4, issue4, specified4, stance4, interpret4, sep = "-")) %>% 
  pivot_longer(cols = round1:round4,
               values_to = "condition",
               names_to = "round") %>% 
  separate(condition, c("actor", "issue", "specification", "stance", "interpretation"), "-") %>% 
  select(id, D1:PT7, attention,
         round, actor, issue, specification, stance, interpretation) %>% 
  mutate(masking = ifelse(actor == "X", "Masked", "Party"),
         stance = ifelse(stance=="correct", 1 ,0),
         interpretation = ifelse(interpretation=="No Stance", 0 ,1),
         distance = 0,
