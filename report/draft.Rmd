...
anonymous: FALSE
abstract: "Information is key to inform the behavior of citizens, and thereby for the social scientists studying them. 
#This first sentence is a bit strange: information to inform behavior? I am not sure what it means, so also not sure how to rephrase..
The democratization of data has lead to numerous possibilities for gathering and analyzing textual data. These enourmous amounts of data are typically handled by machine learning techniques to classify into meaningful variables. The performance of these models is evaluated based on a gold standard, created by human annotators. Having a high level of agreement between these annotators is key, but some
#some what?
suggest personal characteristics of annotators, like political ideology or knowledge, interfere. We show in two pre-registered experiments that XXX. Thereby [contribution]."
...
#possibility to is ungrammatical
In particular, advances in Natural Language Processing (NLP) have made it possible to automatically analyze large quantities of data using machine learning [e.g., see [ADD REF, Bender does not make sense here] @wei2023overview].
...
A recent study     demonstrates that using crowd-coding platforms is a good way to collect such golden standards without too much disagreement.
...
This is different for Dutch, it is a language spoken by a smaller community, typically first language speakers, yet still an often-enough researched case in computational text analysis in the social science [@baden2022three; Dolinsky et al. 2023].
...
@röttger2021 introduce the idea of an explicitly subjective annotation paradigm existing in addition to one focussed on one label and "ground truth".
...
This implies that ideological distance can indeed influence how annotators infer stance, but only if the sentence leaves enough room for interpretation.
...
In other words, when annotators inferred the stance of the party on the issue based on prior knowledge or beliefs about the party.
...
Although most of our hypotheses were not supported, our findings for H1a and H2b in particular pose important implications.
...
But if the sentence refers to the issue without making a clear stance (e.g., many immigrants are crossing our borders) we do see a clear effect of ideological distance.
...
It is when the stance is underspecified that perspectivism [@basile2021toward; @havens2022beyond] comes into play.
...
Our results imply that political ideology is indeed a relevant dimension in shaping how an annotator perceives political stances. 

This means that to improve stance detection models, we need to critically evaluate how we create our gold standards.
...
Even if a good amount of crowd coders is used, it cannot blindly be assumed that this provides a diverse and balanced reflection of political ideologies in society, as we also observed in our samples (Table 2). 
